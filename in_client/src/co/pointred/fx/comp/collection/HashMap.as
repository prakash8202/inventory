package  co.pointred.fx.comp.collection
{
	...
	public class HashMap implements IMap
	{
		...
		public function removeAndUpdate(key:int) : void
		{
			var keyToRemove:int = key;
			
			delete map[key];
			
			var oldArray:Array = getKeys();
			
			for(var ii:int = 0; ii<oldArray.length; ii++)
			{
				var keyVal:int = parseInt(oldArray[ii]);
				
				if(keyVal > keyToRemove){
					put((keyVal - 1), map[keyVal]);
					remove(keyVal);
				}
